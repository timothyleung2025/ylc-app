...
type ParticipantSession = { currentParticipant:YlcParticipant|null; ready:boolean; signInVersion:number; signIn:(code:string)=>YlcParticipant|null; signOut:()=>void };
...
export function ParticipantProvider({children}:{children:React.ReactNode}){
 ...
 const value=useMemo<ParticipantSession>(()=>({currentParticipant,ready,signInVersion,signIn(code){const participant=findParticipantByAccessCode(code);if(!participant)return null;setCurrentParticipant(participant);setSignInVersion(version=>version+1);sessionStorage.setItem(SESSION_KEY,JSON.stringify(participant));return participant},signOut(){setCurrentParticipant(null);sessionStorage.removeItem(SESSION_KEY)}}),[currentParticipant,ready,signInVersion]);
