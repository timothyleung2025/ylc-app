...
export async function POST(request: Request) {
  ...
  const { data, error } = await ctx.supabase!.from("announcements").insert({ title: body.title.trim(), message: isLink ? null : body.message.trim(), link_url: isLink ? body.link_url.trim() : null, category: body.category, is_pinned: Boolean(body.is_pinned) }).select().single();
...
export async function PATCH(request: Request) {
  ...
  const update: { title?: string; message?: string | null; link_url?: string | null; category?: AnnouncementCategory; is_pinned: boolean } = { is_pinned: body.is_pinned };
